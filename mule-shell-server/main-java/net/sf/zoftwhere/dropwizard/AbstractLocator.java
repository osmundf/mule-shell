...
public abstract class AbstractLocator<E, I extends Serializable> extends AbstractDAO<E>
	implements TransactionalSession
{
	...
	public static <V, I, E> Optional<E> tryFetchEntity(V value, final Function<V, Optional<I>> parser,
	...
	protected <T> Optional<T> tryFetchSingleResult(String subName, Function<Query<T>, Optional<T>> routine,
		Class<T> resultType)
	{
		final var name = prefix + "." + subName;
		try (Session session = sessionProvider.get()) {
	...
	protected <T> Optional<List<T>> tryFetchResult(String subName, Function<Query<T>, Query<T>> routine,
		Class<T> resultType)
	{
		final var name = prefix + "." + subName;
		try (Session session = sessionProvider.get()) {
